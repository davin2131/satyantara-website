...
import { formatIsoDateShort } from "@/lib/bookingDates";
...
export function CartDrawer() {
  ...
  function buildWhatsappMessage(): string {
    ...
    items.forEach((it, idx) => {
      ...
      const datePart = it.bookingDate
        ? ` (${formatIsoDateShort(it.bookingDate)})`
        : "";
      lines.push(
        `${idx + 1}. ${it.name} x${it.qty}${datePart} — ${formatIDR(subtotal)}`,
      );
    });
