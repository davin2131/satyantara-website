...
export function StoryCard({ index, title, subtitle, price, delay = 0 }: Props) {
  return (
    <Reveal delay={delay}>
      <article className="group relative overflow-hidden rounded-3xl border border-gold-500/20 bg-coffee-800/60 p-3 transition-all duration-700 hover:-translate-y-1 hover:border-gold-400/60 hover:shadow-[0_30px_60px_-20px_rgba(212,162,78,0.25)]">
        <StoryArt index={index} />
        <div className="px-4 pb-4 pt-4">
          <h3 className="font-display text-2xl text-cream group-hover:text-gold-200 transition-colors">
            {title}
          </h3>
          {subtitle && (
